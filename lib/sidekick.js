...
const addResponseViewData = require('../middleware/add-response-view-data');
...
const loadUserFromApiKey = require('../middleware/load-user-api-key');
const loadUserFromSession = require('../middleware/load-user-session');
...
const paths = {
	apiControllers: `${basePath}/controller/api`,
	frontEndControllers: `${basePath}/controller/front-end`,
...
module.exports.defaults = {
	...
	database: 'postgres://localhost:5432/pa11y_sidekick',
...
function sidekick(options) {
	...
	dashboard.database = createDatabase(dashboard);
	dashboard.model = loadModels(dashboard);
	dashboard.app = createExpressApplication(dashboard);

	// The settings property always has to be a valid
	// Settings model instance, so we create a dummy one.
	dashboard.settings = new dashboard.model.Settings({
		data: {}
	});
...
// This function creates a Bookshelf database client, which
// is used by the models and migrations. The connection string
// is passed in as an option
function createDatabase(dashboard) {
...
function loadModels(dashboard) {
	const models = {};
	values(requireAll(paths.models)).forEach(initModel => initModel(dashboard, models));
	return models;
}
...
function loadControllers(dashboard) {
	loadApiControllers(dashboard);
	loadFrontEndControllers(dashboard);
}

// This function loads all API controllers, as well as
// setting up the relevant error handlers
function loadApiControllers(dashboard) {
	dashboard.app.use('/api', requireHeader('User-Agent'));
	dashboard.app.use('/api', loadUserFromApiKey(dashboard));
	values(requireAll(paths.apiControllers)).forEach(initController => initController(dashboard));
	dashboard.app.use('/api', notFound());
	dashboard.app.use('/api', handleErrors.json(dashboard));
}

// This function loads all front end controllers, as well as
// setting up the relevant error handlers
function loadFrontEndControllers(dashboard) {

	// Set up session middleware, backed by the database. This
	// is not used by the API which is why it's here
	...
	dashboard.app.use(addResponseViewData(dashboard));
	dashboard.app.use(loadUserFromSession(dashboard));

	values(requireAll(paths.frontEndControllers)).forEach(initController => initController(dashboard));
	dashboard.app.use(notFound());
	dashboard.app.use(handleErrors.html(dashboard));
}
...
function createMigration(dashboard, name) {
	return dashboard.database.knex.migrate.make(name, databaseMigrationConfig);
}
...
function migrateUp(dashboard) {
	return dashboard.database.knex.migrate.latest(databaseMigrationConfig);
}

// A simple proxy function to rollback a Knex migration with
// our migration config passed in. Returns a Promise
function migrateDown(dashboard) {
	return dashboard.database.knex.migrate.rollback(databaseMigrationConfig);
}

// A simple proxy function to seed the database with Knex and
// our seed config passed in. Returns a Promise
function seedDatabase(dashboard, directory) {
	return dashboard.database.knex.seed.run({
		directory: directory ? directory : path.join(paths.seeds, 'demo')
	});
}
